        ...
        try:
            ...
            if not Path(f"{directory}/repos_stats.csv").is_file():
                with open(f"{directory}/repos_stats.csv", "a+") as f:
                    f.write(
                        f"Repo,Date,Stars,Forks,Subscribers,Contributors\n"
                    )
            if not Path(
                f"{directory}/{repo_name.replace('/', '_')}_repo_stats.csv"
            ).is_file():
                with open(
                    f"{directory}/{repo_name.replace('/', '_')}_repo_stats.csv",
                    "a+",
            ...
            with open(f"{directory}/repos_stats.csv", "a+") as f:
                f.write(
                    f"{repo_name},{auj},{stargazers},{forks},{subscribers},{contributors}\n"
                )
            with open(
                f"{directory}/{repo_name.replace('/', '_')}_repo_stats.csv",
                "a+",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script extracting statistics from github repos"
    )
    parser.add_argument(
        "--debug",
    ...
    parser.add_argument(
        "-f",
        "--file",
        help="File containing the repos (default : sample file containing popular repos)",
        type=str,
    )
